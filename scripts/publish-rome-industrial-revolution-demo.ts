...
const FINAL_VIDEO_PATH = join(
  HIVEMIND_ROOT,
  "FINAL DEMO VIDEOS",
  "9-rome-industrial-revolution-scenario-compressed-under-30mb.mp4",
);

const POST_ID = "p_brainjuice-onboarding-rome-industrial-revolution";
const POSTHOG_FLAG_KEY = "onboarding-brainjuice-demo-post-ids";
const REPLACE_INDEX = 9;
...
const SOURCE_LABEL = "rome-industrial-revolution-scenario-demo";

const POST_TITLE = "Could You Industrialize Ancient Rome?";
...
const COMMENTS = [
  "The best part is that it does not pretend one genius invention changes everything overnight.",
  "The cylinder tolerance point is such a good explanation for why steam power was harder than it sounds.",
  "Rome having engineers but not the full industrial ecosystem is the key idea here.",
  "I like how the video separates prototypes from repeatable manufacturing.",
  "The labor incentive angle is brutal but important. Machines only spread when they solve the right economic problem.",
  "This made the Industrial Revolution feel less like a single invention and more like a stack of conditions.",
  "The mine pump failing because of seals and corrosion is exactly the kind of detail that makes it believable.",
  "Ancient Rome plus modern engineering is a great setup, but the supply chain lesson is the real payoff.",
  "The Hero engine scene is a nice reminder that impressive tech can still be commercially useless.",
  "Coal, tools, markets, standards, institutions. That list explains more than any one steam engine drawing.",
  "The skeleton engineer makes it fun without turning the history into a joke.",
  "This is a better answer than just saying yes or no. It shows what would actually block the revolution.",
];
...
function initFirebaseForTarget(target: PublishTarget): {
  ...
  const appName = `rome-industrial-revolution-demo-${target.name}`;
...
async function main() {
  ...
  console.log("[rome-industrial-revolution-demo] publishing", {
  ...
  for (const target of targets) {
    console.log(`[rome-industrial-revolution-demo] target ${target.name}: upload artifact`);
    ...
    console.log(`[rome-industrial-revolution-demo] target ${target.name}: upsert DB rows`);
    if (!options.dryRun) await upsertDatabaseRows(target, upload.artifactUrl);
    console.log(`[rome-industrial-revolution-demo] target ${target.name}: update PostHog`);
